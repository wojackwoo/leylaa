...
export default function PollsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    const { error: contactError } = await supabase.from("contacts").upsert(
      { session_id: getSessionId(), whatsapp: whatsapp.trim() },
            ...
            <div className="text-center">
              ...
              <div className="mt-12 grid gap-5 sm:grid-cols-2">
                <button onClick={() => chooseStart(options.find((o) => o.poll_id === 1 && o.label.toLowerCase() === "cinema")!)} disabled={loading} className="group rounded-3xl border border-black/10 bg-white p-8 text-left transition duration-300 hover:-translate-y-1 hover:shadow-xl disabled:opacity-60">
                  <div className="mb-8 text-6xl">🎬</div><h2 className="text-3xl font-semibold">Cinema</h2><p className="mt-3 leading-6 text-black/55">Pick a movie, choose when you'd like to go, and we'll keep you updated.</p><div className="mt-8 text-sm font-medium">Choose Cinema →</div>
                </button>
                <button onClick={() => chooseStart(options.find((o) => o.poll_id === 1 && o.label.toLowerCase() === "party")!)} disabled={loading} className="group rounded-3xl border border-black/10 bg-white p-8 text-left transition duration-300 hover:-translate-y-1 hover:shadow-xl disabled:opacity-60">
                  <div className="mb-8 text-6xl">🎉</div><h2 className="text-3xl font-semibold">Party</h2><p className="mt-3 leading-6 text-black/55">Tell us your vibe, choose the music, and help us shape the next night.</p><div className="mt-8 text-sm font-medium">Choose Party →</div>
                </button>
              </div>
            </div>
          )}
          ...
          {activePollId && (
            <div>
              <div className="mb-12 text-center">
                <div className="mb-6 text-6xl">{isCinema ? "🎬" : "🎉"}</div>
                ...
                {currentOptions.map((option) => {
                  ...
                  return (
                    <button key={option.id} disabled={loading} onClick={() => step === "cinema-movie" ? chooseCinemaMovie(option) : step === "cinema-time" ? chooseCinemaTime(option) : step === "party-vibe" ? choosePartyVibe(option) : step === "party-music" ? choosePartyMusic(option) : choosePartyTime(option)} className="group overflow-hidden rounded-3xl border border-black/10 bg-white text-left transition duration-300 hover:-translate-y-1 hover:shadow-xl disabled:opacity-60">
                      {image ? <div className={`overflow-hidden ${step === "cinema-movie" ? "aspect-[2/3]" : "aspect-[4/3]"}`}><img src={image} alt={option.label} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /></div> : <div className="flex aspect-[4/3] items-center justify-center bg-[#eee9e1] text-6xl">{option.emoji ?? (isCinema ? "🎬" : "🎉")}</div>}
              ...
              <div className="mt-10 text-center"><button onClick={goBack} className="rounded-full border border-black/15 px-6 py-3 text-sm font-medium transition hover:bg-black hover:text-white">← Go back</button></div>
            </div>
          )}
